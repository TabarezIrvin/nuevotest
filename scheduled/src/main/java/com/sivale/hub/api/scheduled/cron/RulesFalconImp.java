...
import com.sivale.hub.api.scheduled.constants.fields.mandatorys.FieldsDefaultRulesFalcon;
...
/**
 * Description: This class is used for ... Class created on 29 ago. 2019.
 * 
 * @author Gulliver Paredes Aguirres [gulliver.paredes.eon@gmail.com]
 */
@ISchedulerJob
public class RulesFalconImp {

    static final Logger log = LoggerFactory.getLogger(RulesFalconImp.class);
    ...
    @Value("${sivale.ruta.origen.reglas.falcon.win}")
    private String windowsOrigin;

    @Value("${sivale.ruta.destino.reglas.falcon.win}")
    private String windowsDestination;

    @Value("${sivale.ruta.origen.reglas.falcon.linux}")
    private String linuxOrigin;

    @Value("${sivale.ruta.destino.reglas.falcon.linux}")
    private String linuxDestination;

    @Value("${sivale.nombre.tabla.reglas.falcon}")
    ...
    private String cronJobName = "cronJobRulesFalcon";
	...
	FieldsDefaultRulesFalcon fieldsRulesFalcon = new FieldsDefaultRulesFalcon();
	...
	if (existingFiles != null) {
		...
		operationPerformed |= cef.processExcel(destination, existingFiles[i],
			this.shapeFields(fieldsRulesFalcon), paths, factor);
	    }
	}
	InsertInPostgres postgres = new InsertInPostgres();
	if (operationPerformed) {
	    operationPerformed = false;
	    operationPerformed |= postgres.insertFile(this.dataForm(), fieldsRulesFalcon.positionFieldBD(), tableName,
		    pathAcceptedFiles, pathRejectedFiles);
	}
	if (!operationPerformed) {
	...
	this.insertInBitacora(operationPerformed ? Constantes.CORRECTO : Constantes.ERROR,
		((fin - start) / Constantes.MIL), operationPerformed ? null : ConstantesMensajes.ERROR, null);
	log.info(ConstantesMensajes.LOG_FIN, tableName, (int) ((fin - start) / Constantes.MIL));
    }

    /**
     * 
     * Method used for ... Returns an object of type List<HashMap>. Method created
     * on 10 sep. 2019, by Gulliver Paredes Aguirres
     * [gulliver.paredes.eon@gmail.com]
     * 
     * @param fieldsRulesFalcon
     * @return
     */
    @SuppressWarnings("rawtypes")
    private List<HashMap> shapeFields(FieldsDefaultRulesFalcon fieldsRulesFalcon) {
	List<HashMap> fields = new ArrayList<>();
	fields.add(fieldsRulesFalcon.fieldTypeExcel());
	fields.add(fieldsRulesFalcon.mappingDBExcel());
	fields.add(fieldsRulesFalcon.positionFieldBD());
	return fields;
    }

    /**
     * 
     * Method used for ... Returns an object of type List<String>. Method created on
     * 10 sep. 2019, by Gulliver Paredes Aguirres [gulliver.paredes.eon@gmail.com]
     * 
     * @return
     */
    ...
    /**
     * 
     * Method used for ... Returns an object of type String[]. Method created on 10
     * sep. 2019, by Gulliver Paredes Aguirres [gulliver.paredes.eon@gmail.com]
     * 
     * @return
     */
    ...
    /**
     * 
     * Method used for ... Returns an object of type boolean. Method created on 10
     * sep. 2019, by Gulliver Paredes Aguirres [gulliver.paredes.eon@gmail.com]
     * 
     * @param start
     * @param origin
    ...
    /**
     * 
     * Method used for ... Returns an object of type void. Method created on 10 sep.
     * 2019, by Gulliver Paredes Aguirres [gulliver.paredes.eon@gmail.com]
     * 
     * @param status
     * @param time
     * @param error
