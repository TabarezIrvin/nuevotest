...
import com.sivale.hub.api.scheduled.constants.fields.mandatorys.FieldsDefaultKeyMonitorReceptor;
 ...
 */
@ISchedulerJob
public class KeyMonitorReceptorImp {

    static final Logger log = LoggerFactory.getLogger(KeyMonitorReceptorImp.class);
    ...
    @Value("${sivale.ruta.origen.keymonitor.receptor.win}")
    private String windowsOrigin;

    @Value("${sivale.ruta.destino.keymonitor.receptor.win}")
    private String windowsDestination;
    
    @Value("${sivale.ruta.origen.keymonitor.receptor.linux}")
    private String linuxOrigin;

    @Value("${sivale.ruta.destino.keymonitor.receptor.linux}")
    private String linuxDestination;
    
    @Value("${sivale.nombre.tabla.keymonitor.receptor}")
    ...
    private String cronJobName = "cronJobKeymonitorReceptor";
    private String pathC = null;
    private String pathPython = null;
    
    @Scheduled(cron = "${cronExpression}")
    public void cronJobKeymonitorReceptor() {
	...
	FieldsDefaultKeyMonitorReceptor fieldsDefaultKeyMonitorReceptor = new FieldsDefaultKeyMonitorReceptor();
	...
	if (existingFiles != null) {
	    ...
	    cef.setPositionDBFieldTarjeta(Constantes.DIECIOCHO);
	    for (int i = 0; i < existingFiles.length; i++) {
		operationPerformed |= cef.processExcel(destination, existingFiles[i], this.shapeFields(fieldsDefaultKeyMonitorReceptor),
			paths, factor);
	    }
	...
	if (operationPerformed) {
	    ...
	    operationPerformed |= postgres.insertFile(this.dataForm(), fieldsDefaultKeyMonitorReceptor.positionFieldBD(),
    ...
    private List<HashMap> shapeFields(FieldsDefaultKeyMonitorReceptor fieldsDefaultKeyMonitorReceptor) {
	List<HashMap> fields = new ArrayList<>();
	fields.add(fieldsDefaultKeyMonitorReceptor.fieldTypeExcel());
	fields.add(fieldsDefaultKeyMonitorReceptor.mappingDBExcel());
	fields.add(fieldsDefaultKeyMonitorReceptor.positionFieldBD());
	return fields;
    }
