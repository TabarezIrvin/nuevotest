...
import com.sivale.hub.api.scheduled.constants.fields.mandatorys.FieldsDefaultRejectionsDayCVV;
 ...
 */
@ISchedulerJob
public class RejectionsDayCVVImp {
    
    static final Logger log = LoggerFactory.getLogger(RejectionsDayCVVImp.class);
    ...
    @Value("${sivale.ruta.origen.rechazos.dia.cvv.win}")
    private String windowsOrigin;

    @Value("${sivale.ruta.destino.rechazos.dia.cvv.win}")
    private String windowsDestination;
    
    @Value("${sivale.ruta.origen.rechazos.dia.cvv.linux}")
    private String linuxOrigin;

    @Value("${sivale.ruta.destino.rechazos.dia.cvv.linux}")
    private String linuxDestination;
    
    @Value("${sivale.nombre.tabla.rechazos.dia.cvv}")
    ...
    private String cronJobName = "cronJobRejectionsDayCVV";
    private String pathC = null;
    private String pathPython = null;
    
    @Scheduled(cron = "${cronExpression}")
    public void cronJobRejectionsDayCVV() {
	...
	FieldsDefaultRejectionsDayCVV fieldsRejectionsDayCVV = new FieldsDefaultRejectionsDayCVV();
	...
	if (existingFiles != null) {
		...
		operationPerformed |= cef.processExcel(destination, existingFiles[i], this.shapeFields(fieldsRejectionsDayCVV),
			paths, factor);
	    }
	}
	...
	if (operationPerformed) {
	    ...
	    operationPerformed |= postgres.insertFile(this.dataForm(), fieldsRejectionsDayCVV.positionFieldBD(),
    ...
    private List<HashMap> shapeFields(FieldsDefaultRejectionsDayCVV fieldsRejectionsDayCVV) {
	List<HashMap> fields = new ArrayList<>();
	fields.add(fieldsRejectionsDayCVV.fieldTypeExcel());
	fields.add(fieldsRejectionsDayCVV.mappingDBExcel());
	fields.add(fieldsRejectionsDayCVV.positionFieldBD());
	return fields;
    }
