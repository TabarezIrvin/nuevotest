...
import com.sivale.hub.api.scheduled.constants.fields.mandatorys.FieldsDefaultCpConcentrate;
 ...
 */
@ISchedulerJob
public class CpConcentrateImp {

    static final Logger log = LoggerFactory.getLogger(CpConcentrateImp.class);
    ...
    @Value("${sivale.ruta.origen.concentrado.cp.win}")
    private String windowsOrigin;

    @Value("${sivale.ruta.destino.concentrado.cp.win}")
    private String windowsDestination;
    
    @Value("${sivale.ruta.origen.concentrado.cp.linux}")
    private String linuxOrigin;

    @Value("${sivale.ruta.destino.concentrado.cp.linux}")
    private String linuxDestination;
    
    @Value("${sivale.nombre.tabla.concentrado.cp}")
    ...
    private String cronJobName = "cronJobCpConcentrate";
    private String pathC = null;
    private String pathPython = null;
    
    @Scheduled(cron = "${cronExpression}")
    public void cronJobCpConcentrate() {
	...
	FieldsDefaultCpConcentrate fieldsDefaultCpConcentrate = new FieldsDefaultCpConcentrate();
	...
	if (existingFiles != null) {
	    ...
	    cef.setPositionDBFieldTarjeta(Constantes.CINCO);
	    cef.setPositionDBFieldExtra(Constantes.SEIS);
	    for (int i = 0; i < existingFiles.length; i++) {
		operationPerformed |= cef.processExcel(destination, existingFiles[i], this.shapeFields(fieldsDefaultCpConcentrate),
			paths, factor);
	    }
	}
	...
	if (operationPerformed) {
	    ...
	    operationPerformed |= postgres.insertFile(this.dataForm(), fieldsDefaultCpConcentrate.positionFieldBD(),
     ...
     * @param fieldsDefaultCpConcentrate
     * @return
     */
    @SuppressWarnings("rawtypes")
    private List<HashMap> shapeFields(FieldsDefaultCpConcentrate fieldsDefaultCpConcentrate) {
	List<HashMap> fields = new ArrayList<>();
	fields.add(fieldsDefaultCpConcentrate.fieldTypeExcel());
	fields.add(fieldsDefaultCpConcentrate.mappingDBExcel());
	fields.add(fieldsDefaultCpConcentrate.positionFieldBD());
	return fields;
    }
