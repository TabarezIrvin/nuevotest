...
import com.sivale.hub.api.scheduled.constants.fields.mandatorys.FieldsDefaultBDU;
...
public class BDUImp {
    ...
    static final Logger log = LoggerFactory.getLogger(BDUImp.class);
    ...
    @Value("${sivale.ruta.origen.bdu.win}")
    private String windowsOrigin;

    @Value("${sivale.ruta.destino.bdu.win}")
    private String windowsDestination;
    
    @Value("${sivale.ruta.origen.bdu.linux}")
    private String linuxOrigin;

    @Value("${sivale.ruta.destino.bdu.linux}")
    private String linuxDestination;
    
    @Value("${sivale.nombre.tabla.bdu}")
    ...
    private String cronJobName = "cronJobBDU";
     ...
     * Method created on 17 sep. 2019, by Gulliver Paredes Aguirres [gulliver.paredes.eon@gmail.com]
     */
    @Scheduled(cron = "${cronExpression}")
    public void cronJobBDU() {
	...
	FieldsDefaultBDU fieldsBDU = new FieldsDefaultBDU();
	...
	if (existingFiles != null) {
		...
		operationPerformed |= cef.processExcel(destination, existingFiles[i], this.shapeFields(fieldsBDU),
			paths, factor);
	    }
	}
	...
	if (operationPerformed) {
	    ...
	    operationPerformed |= postgres.deleteTable(this.dataForm(), tableName);
	}
	if (operationPerformed) {
	    operationPerformed = false;
	    operationPerformed |= postgres.insertFile(this.dataForm(), fieldsBDU.positionFieldBD(),
    ...
    private List<HashMap> shapeFields(FieldsDefaultBDU fieldsBDU) {
	List<HashMap> fields = new ArrayList<>();
	fields.add(fieldsBDU.fieldTypeExcel());
	fields.add(fieldsBDU.mappingDBExcel());
	fields.add(fieldsBDU.positionFieldBD());
	return fields;
    }
