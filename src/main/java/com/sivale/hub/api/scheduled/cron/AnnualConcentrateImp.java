...
import com.sivale.hub.api.scheduled.constants.fields.mandatorys.FieldsDefaultAnnualConcentrate;
 ...
 */
@ISchedulerJob
public class AnnualConcentrateImp {
    
    static final Logger log = LoggerFactory.getLogger(AnnualConcentrateImp.class);
    ...
    @Value("${sivale.ruta.origen.concentrado.anual.win}")
    private String windowsOrigin;

    @Value("${sivale.ruta.destino.concentrado.anual.win}")
    private String windowsDestination;
    
    @Value("${sivale.ruta.origen.concentrado.anual.linux}")
    private String linuxOrigin;

    @Value("${sivale.ruta.destino.concentrado.anual.linux}")
    private String linuxDestination;
    
    @Value("${sivale.nombre.tabla.concentrado.anual}")
    ...
    private String cronJobName = "cronJobAnnualConcentrate";
    private String pathC = null;
    private String pathPython = null;
    
    @Scheduled(cron = "${cronExpression}")
    public void cronJobAnnualConcentrate() {
	...
	FieldsDefaultAnnualConcentrate fieldsDefaultAnnualConcentrate = new FieldsDefaultAnnualConcentrate();
	...
	if (existingFiles != null) {
	    ...
	    cef.setPositionDBFieldTarjeta(Constantes.OCHO);
	    cef.setPositionDBFieldExtra(Constantes.DIEZ);
	    
	    for (int i = 0; i < existingFiles.length; i++) {
		operationPerformed |= cef.processExcel(destination, existingFiles[i], this.shapeFields(fieldsDefaultAnnualConcentrate),
			paths, factor);
	    }
	}
	...
	if (operationPerformed) {
	    ...
	    operationPerformed |= postgres.insertFile(this.dataForm(), fieldsDefaultAnnualConcentrate.positionFieldBD(),
     ...
     * @param fieldsDefaultAnnualConcentrate
     * @return
     */
    @SuppressWarnings("rawtypes")
    private List<HashMap> shapeFields(FieldsDefaultAnnualConcentrate fieldsDefaultAnnualConcentrate) {
	List<HashMap> fields = new ArrayList<>();
	fields.add(fieldsDefaultAnnualConcentrate.fieldTypeExcel());
	fields.add(fieldsDefaultAnnualConcentrate.mappingDBExcel());
	fields.add(fieldsDefaultAnnualConcentrate.positionFieldBD());
	return fields;
    }
