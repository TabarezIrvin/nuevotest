...
import com.sivale.hub.api.scheduled.constants.fields.mandatorys.FieldsDefaultKeyMonitor;
 ...
 */
@ISchedulerJob
public class KeyMonitorCNPImp {

    static final Logger log = LoggerFactory.getLogger(KeyMonitorCNPImp.class);
    ...
    @Value("${sivale.ruta.origen.keymonitorcnp.win}")
    private String windowsOrigin;

    @Value("${sivale.ruta.destino.keymonitorcnp.win}")
    private String windowsDestination;
    
    @Value("${sivale.ruta.origen.keymonitorcnp.linux}")
    private String linuxOrigin;

    @Value("${sivale.ruta.destino.keymonitorcnp.linux}")
    private String linuxDestination;
    
    @Value("${sivale.nombre.tabla.keymonitorcnp}")
    private String tableName;
    
    //Constantes para conexion a BD--------------------
    ...
    private String cronJobName = "cronJobKeymonitorCNP";
    private String pathC = null;
    private String pathPython = null;
    
    @Scheduled(cron = "${cronExpression}")
    public void cronJobKeymonitorCNP() {
	...
	FieldsDefaultKeyMonitor fieldsDefaultKeyMonitor = new FieldsDefaultKeyMonitor();
	...
	if (existingFiles != null) {
	    ...
	    cef.setPositionDBFieldTarjeta(Constantes.SEIS);
	    for (int i = 0; i < existingFiles.length; i++) {
		operationPerformed |= cef.processExcel(destination, existingFiles[i], this.shapeFields(fieldsDefaultKeyMonitor),
			paths, factor);
	    }
	}
	...
	if (operationPerformed) {
	    ...
	    operationPerformed |= postgres.insertFile(this.dataForm(), fieldsDefaultKeyMonitor.positionFieldBD(),
    ...
    private List<HashMap> shapeFields(FieldsDefaultKeyMonitor fieldsDefaultKeyMonitor) {
	List<HashMap> fields = new ArrayList<>();
	fields.add(fieldsDefaultKeyMonitor.fieldTypeExcel());
	fields.add(fieldsDefaultKeyMonitor.mappingDBExcel());
	fields.add(fieldsDefaultKeyMonitor.positionFieldBD());
	return fields;
    }
